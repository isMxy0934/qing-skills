...
### 新功能
- track-ai-plans：重排仪表盘信息架构——先 Plan 身份与风险徽章，再 handoff 下一步主行、摘要、任务、模块影响，变更/审计详情默认折叠
- track-ai-plans：新增手动明/暗/跟随系统主题切换，默认浅色，不再强制跟随系统深色
- track-ai-plans：新增 `planctl.py serve` 命令，一键起本地服务（仅监听 127.0.0.1）并自动打开仪表盘，不用每次手动起 HTTP 服务（仪表盘的 `fetch()` 在 `file://` 协议下会被拦截）

### 重构
- track-ai-plans：任务状态色（蓝/绿/琥珀/红）与模块影响色（紫）拆分；跨设备可交接状态改用中性文案加图标，不再复用状态色
- track-ai-plans：去掉 Phase 图中的模块/文件计数重复展示，统一只在模块影响图显示；增加视觉层级，Changes 与 Audit 默认折叠
- track-ai-plans：列表与依赖图两种视图默认都聚焦同一个"当前" Phase（其余折叠，点击展开），"全部阶段"改为显式的主动选择而非默认态；明确列表/依赖图切换只作用于单个 Phase 内的任务，与始终可见的 Phase 关系图区分开

### 修复
- track-ai-plans：修复阶段列表手风琴的 bug——收起当前已展开的 Phase 卡片时会误触发全部展开；列表自身的展开/收起状态现在与 Phase 关系图的影响范围选择分开维护，不再共用同一个切换逻辑
- track-ai-plans：把"全部阶段"按钮从任务列表工具栏移到它真正作用的模块影响图面板，并且不再影响任务列表的展开/收起状态或任务依赖图的聚焦 Phase
...
### 修复
- track-ai-plans：修复仪表盘把迁移过来的 `done` 计划文件误标为"待完成"的问题；冻结的迁移快照现在会补上 V1 自身完成规则已经保证过的文件匹配结果，而不是让 V2 实时投影通常会填充的逐项 observations 留空

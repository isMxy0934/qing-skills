...
Optional OpenAI Agents metadata (display name, short description, default prompt) lives in `agents/openai.yaml`. Keep it in sync when renaming the skill or changing the default workflow summary.
...
### 4. Build the market-wide prefilter universe first
...
### 7. Switch modes when the user wants tail entry
...
The sample ticker, prices, and dates above are **layout placeholders only**; real rows must use verified codes and anchor timestamps from fetched session data.
...
- `agents/openai.yaml` when wiring this skill into OpenAI Agents (optional)
